...
- **PNG**：使用 `page.screenshot`，支持 `viewport` / `deviceScaleFactor` 等。**只截图 `#card-container`**（缺失即失败）。
...
- 每个 skill 至少包含：`SKILL.md`、`converter.js`、`templates/`、`examples/`、`scripts/`。
...
### 2) `skills/web-to-png`
- 将 URL / HTML（本地或 URL）/ Markdown（本地）导出为 PNG。
- 主题：`default` / `sketch` / `magazine` / `bold` / `poster`
- 默认模式：宽 1080，高度自适应（无 preset）
- 固定模式：使用 preset 固定尺寸，超出内容隐藏并可省略号
- 预设尺寸：  
  - `og` 1200×630  
  - `square` 1080×1080  
  - `story` 1080×1920  
